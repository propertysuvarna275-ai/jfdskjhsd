...
function validateConnectionString(connectionString) {
  if (!connectionString) {
    throw new Error("[AUTH] Variabel lingkungan NEON_DATABASE_URL tidak diatur atau kosong!");
  }
  const lower = connectionString.toLowerCase();
  if (!lower.startsWith("postgres://") && !lower.startsWith("postgresql://")) {
    throw new Error("[AUTH] NEON_DATABASE_URL harus menggunakan prefix postgres:// atau postgresql://");
  }
}
...
function getPool() {
  ...
  if (!pool) {
    try {
      ...
      throw new Error(` [AUTH] Gagal membuat koneksi Neon: ${error.message}`);
    }
  }
  return pool;
}
